...
export default function DiscountPage() {
    ...
    useEffect(() => {
        axios.get('/api/admin/discount').then(response => {
    ...
    return (
        <Layout>
            <Link className="bg-blue-900 text-white py-1 px-2 rounded-md " href={'/admin/discount/new'}>
                Add new discount
            </Link>
            <h1 className="mt-2 text-blue-900 text-lg">Number of discounts: {discounts.length}</h1>
                    ...
                    {discounts.slice(0, showMore * 10 < discounts.length ? showMore * 10 : discounts.length).map(discount => (
                        <tr key={discount._id}>
                            ...
                            <td className='flex gap-1'>
                                <Link href={'/admin/discount/edit/' + discount._id}>
                                ...
                                <Link href={'/admin/discount/delete/' + discount._id}>
                                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" data-slot="icon" class="w-4 h-4">
